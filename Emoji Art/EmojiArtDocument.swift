...
extension UTType {
    static let emojiart = UTType(exportedAs: "gl0ukhov.Emoji-Art")
}
...
class EmojiArtDocument: ReferenceFileDocument {
    ...
    private func fetchBackgroundImage() async {
        if let url = emojiArt.background {
            ...
            do {
                ...
                background = .failed("Couldn't set bakcground: \(error.localizedDescription)")
            }
        } else {
            background = .none
        }
    ...
    func changePositionEmoji(emoji: Emoji.ID, offset: CGSize, undoWith undoManager: UndoManager? = nil) {
    ...
    func changeZoomEmoji(emoji: Emoji.ID, zoom: CGFloat, undoWith undoManager: UndoManager? = nil) {
        undoablyPerform("Zoom \(emoji)", with: undoManager) {
            if let index = emojiArt.emojis.firstIndex(emojiID: emoji) {
    ...
    func deleteEmoji(emoji: Emoji.ID, undoWith undoManager: UndoManager? = nil) {
